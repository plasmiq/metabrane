class WorkingPair < ActiveRecord::Base
  ...
  def entangled
    WorkingPair.where( "relation = ? or (substrate1_id = ? or substrate2_id = ?) or (substrate1_id = ? or substrate2_id = ?)", relation, substrate1.id, substrate2.id, substrate2.id, substrate1.id)
  end
