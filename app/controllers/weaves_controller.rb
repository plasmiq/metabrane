class WeavesController < ApplicationController
  ...
  def index 
    @page = params[:page] || 1
    @per_page = 4
    weaves = WorkingPair.select('working_pairs.id, working_pairs.substrate1_id, working_pairs.substrate2_id, max(working_pairs.id) as created_at').group("substrate1_id, substrate2_id").newest.paginate(:page => @page, :per_page => @per_page)
